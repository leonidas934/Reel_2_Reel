...
double speed;
int input_speed;
const unsigned long printInterval = 100;  // Print every nb ms
 ...
 void main_loop(){
  ...
  case on:{
    ...
    if (now - lastPrintTime >= printInterval ) { //print intervall = nb ms
      lastPrintTime = now;
      print_results(now-kernel.start_time, 0, 0, 0, input_breake_start_pos, rest_pos, _board._pos_sensor.get_pos(), test);
    }
    break;
  }
  ...
  case running:{
    double time = kernel.get_time();
    unsigned long now = millis();
    if (now - lastSpeedTime >= 50) { //speed interval = 50ms
      lastSpeedTime = now;
    ...
    if (now - lastPrintTime >= printInterval ) { //print intervall = nb ms
      lastPrintTime = now;
      print_results(now-kernel.start_time, moving_speed, speed, input_speed, input_breake, des_pos, pos, test);
    }
    break;
    }
  }
